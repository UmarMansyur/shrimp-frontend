<template>
  <button class="btn btn-outline-warning btn-sm me-2 mb-3" data-bs-target="#anco-modal" data-bs-toggle="modal"
    @click="() => { getFeed(new Date().toISOString().split('T')[0]); initializeChoices(); }">
  ...
  <Modal modal="modal-lg modal-dialog-scrollable" modal-id="anco-modal" title="Cek Anco">
    <div class="row">
      <div class="col-12">
        <div class="alert alert-warning alert-top-border">
          Jika pakan yang diberikan tidak terdeteksi. Silahkan klik tanggal pemberian pakan dan pilih tanggal. Jika tetap
          tidak terdeteksi, silahkan refresh halaman.
        </div>
      </div>
    ...
    <div class="row" v-show="pond.is_start">
      <div class="col-md-6 mb-3">
        ...
        <VueDatePicker light="true" auto-apply format="yyyy-MM-dd" v-model="feed_stock" :enable-time-picker="false"
          :class="{
      ...
      <div class="col-md-6 mb-3">
        ...
        <select name="time-feed" id="time-feed" class="form-select" v-model="time" @change="getTime"
          :class="{ 'is-invalid': !time_err.valid && time_err.dirty }">
      ...
      <div class="col-md-12 mb-3">
        ...
        <textarea name="desc" id="desc" cols="30" rows="10" class="form-control" v-model="desc"
          :class="{ 'is-invalid': !desc_err.valid && desc_err.dirty }"></textarea>
      </div>
      ...
      <div class="col-6 mb-3 text-end">
        <button class="btn btn-success" data-bs-dismiss="modal" :disabled="!meta.valid" @click="save">
...
watch(time, async (value) => {
  if (value) {
    ...
    if (response) {
      amountAnco.value = response.anco_amount;
    }
  }
});
